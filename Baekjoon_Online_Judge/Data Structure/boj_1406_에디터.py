...
def myEditor_failed():
    ...
    order = []
    for _ in range(m):
        order.append(stdin.readline().rstrip())

    cursor = len(text)
    for o in order:
        if o == 'L' and cursor > 0:
            cursor -= 1
        elif o == 'D' and cursor < len(text):
            cursor += 1
        elif o == 'B' and cursor > 0:
            if cursor == 1:
                text = text[1:]
            ...
            if cursor == 0:
                ...
                text = text[0:cursor] + [o[-1]] + text[cursor:]
            cursor += 1
        # print(cursor, text)
...
# 힌트 : 커서 왼쪽/오른쪽 구분해서 리스트 두개로 구현해보기


def myEditor_try2():
    from sys import stdin
    curLeftL = list(stdin.readline().rstrip())
    curRightL = []
    m = int(stdin.readline())
    # order = [ stdin.readline().rstrip() for _ in range(m) ]

    for i in range(m):
        a = stdin.readline().rstrip()

        # if a == 'L' and len(curLeftL) > 0 :
        if a == 'L' and curLeftL:  # 있을때만 확인하면 되니까 len으로 안해도 됨
            curRightL.append(curLeftL.pop())

        elif a == 'D' and curRightL:
            curLeftL.append(curRightL.pop())

        elif a == 'B' and curLeftL:
            curLeftL.pop()

        elif a[0] == 'P':
            curLeftL.append(a[-1])

    print(''.join(curLeftL) + ''.join(reversed(curRightL)))
